...
- **Remaining:** (1) **run** the public/external non-self-operated miner test (blocked on operator collecting the miner inputs + firewall handoff); (2) remote slow-cpuminer low-devnet-height PoW caveat (remote CPU connects/authorizes/receives work but does not land a diff-1 share in-window — environment limitation, not Phase 20 logic).
