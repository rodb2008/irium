...
fn main() -> Result<(), String> {
    ...
    match cmd {
        "verify" => {
            if args.len() < 6 {
                return Err("verify requires: <height> <txid> <index> <proof_hex_comma_separated>".to_string());
            }
            ...
            if valid {
                ...
                println!("SPV proof INVALID for txid {} at height {}", txid_hex, height);
            }

            // Demonstrate header-chain validation for this single header.
            let header_bits = u32::from_str_radix(
                jb.header.bits.trim_start_matches("0x"),
                16,
            )
