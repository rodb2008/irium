...
pub fn apply_btc_header_batch(
    ...
    for (i, header) in headers.iter().enumerate() {
        ...
        if btc_headers.contains_key(&hash) {
            ...
            // Closes the cache/apply tip race that caused carrier blocks
            // to fail at apply time when the relay tip advanced between
            // template build and mine. Pre-activation: legacy reject.
            ...
            if apply_idempotency_active {
                let entry = btc_headers.get(&hash).expect("contains_key true");
                if entry.height != height || entry.header != *header {
                    return Err(format!(
                        "apply_btc_header_batch: header {} mismatches known chain (height/bytes)",
                        i
                    ));
                }
                prev_hash = hash;
